...
/** @var $form AdministratorIndexForm */

/** @var $this AdministratorController */
$this->pageTitle = Yii::t('wojia', 'Administrators');
...
$actionWidth = 16;
if ($user->checkAccess('administrator:view'))
    $actionWidth += 40;
if ($user->checkAccess('administrator:update'))
    $actionWidth += 40;
if ($user->checkAccess('administrator:active'))
    $actionWidth += 40;

/** @var $clientScript CClientScript */
$clientScript = Yii::app()->clientScript;
$clientScript->registerCss('administrator-index', '
.table-column-actions {width: ' . $actionWidth . 'px}
');
$clientScript->registerScript('administrator-index', '
...
?>
    ...
    <div class="page-header">
        <h1><?php echo Yii::t('wojia', 'Administrators'); ?></h1>
    </div>
...
<?php if ($dataProvider->itemCount): ?>
    <table class="table table-striped">
        <thead>
        <tr>
            <th class="table-column-id"><?php echo Yii::t('wojia', 'Administrator ID'); ?></th>
            <th class="table-column-name"><?php echo Yii::t('wojia', 'Administrator name'); ?></th>
        ...
        <?php foreach ($dataProvider->data as $data): ?>
            <tr <?php if (!$data->active): ?>class="danger"<?php endif; ?>>
                ...
                <td>
                    <div class="btn-group">
                        <?php if ($user->checkAccess('administrator:view')): ?>
                            <a class="btn btn-default"
                               href="<?php echo $this->createUrl('administrator/view', array('id' => $data->id)); ?>"
                               title="<?php echo Yii::t('wojia', 'View'); ?>">
                                <span class="glyphicon glyphicon-eye-open"></span>
                            </a>
                        <?php endif; ?>
                        <?php if ($user->checkAccess('administrator:update')): ?>
                            <a class="btn btn-default"
                               href="<?php echo $this->createUrl('administrator/update', array('id' => $data->id)); ?>"
                               title="<?php echo Yii::t('wojia', 'Edit'); ?>">
                                <span class="glyphicon glyphicon-edit"></span>
                            </a>
                        <?php endif; ?>
                        <?php if ($user->checkAccess('administrator:active')): ?>
                            <a class="btn btn-danger"
                               href="<?php echo $this->createUrl('administrator/active', array('id' => $data->id)); ?>"
...
<?php else: ?>
    <p>
        <?php echo Yii::t('wojia', 'No :item.', array(
            ':item' => Yii::t('wojia', 'administrators'),
        )); ?>
    </p>
<?php endif; ?>