...
/** @var $form ManagerIndexForm */

/** @var $this ManagerController */
$this->pageTitle = $title = Yii::t('wojia', 'Managers');
...
$actionWidth = 16;
if ($user->checkAccess('manager:view'))
    $actionWidth += 40;
if ($user->checkAccess('manager:update'))
    $actionWidth += 40;
if ($user->checkAccess('manager:active'))
    $actionWidth += 40;

/** @var $clientScript CClientScript */
$clientScript = Yii::app()->clientScript;
$clientScript->registerCss('manager-index', '
.table-column-actions {width: ' . $actionWidth . 'px}
');
$clientScript->registerScript('manager-index', '
...
<?php if ($dataProvider->itemCount): ?>
    <table class="table table-striped">
        <thead>
        <tr>
            <th class="table-column-id"><?php echo Yii::t('wojia', 'Manager ID'); ?></th>
                    ...
                    <div class="btn-group">
                        <?php if ($user->checkAccess('manager:view')): ?>
                            <a class="btn btn-default"
                               href="<?php echo $this->createUrl('manager/view', array('id' => $data->id)); ?>"
                               title="<?php echo Yii::t('wojia', 'View'); ?>">
                                <span class="glyphicon glyphicon-eye-open"></span>
                            </a>
                        <?php endif; ?>
                        <?php if ($user->checkAccess('manager:update')): ?>
                            <a class="btn btn-default"
                               href="<?php echo $this->createUrl('manager/update', array('id' => $data->id)); ?>"
                               title="<?php echo Yii::t('wojia', 'Edit'); ?>">
                                <span class="glyphicon glyphicon-edit"></span>
                            </a>
                        <?php endif; ?>
                        <?php if ($user->checkAccess('manager:active')): ?>
                            <a class="btn btn-danger"
                               href="<?php echo $this->createUrl('manager/active', array('id' => $data->id)); ?>"
